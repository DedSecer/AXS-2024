...
class Solution:
    ...
    POSE_CLOSE_MICROWAVE = (np.array([-0.05931371154832413 + 0.24, 0.10621326743872146, 0.052710225767309826]), np.array([0.00476639,  0.08115882, 0.05789078, 0.99500713]))
    ...
    ARM_POSE_STANDARD_MOVING = (np.array([0.35, 0.00, 0.219]), np.array([0.0, 0.0, 0.0, 1.0]))
    ...
    END_POSE = (np.array([-1.2317611908186263 + 0.2, -0.9177336410440479, -0.2374505629662929]), 
    ...
    def grasp_handle(self):
        ...
        cloud = self.base_cloud(_image, _depth, self.camera.INTRINSIC, self.CAMERA_SHIFT, self.arm.end_pose)
        grasp_position = cloud[ _bbox[0], _bbox[1]][:3]
        grasp_position[0] += 0.014
        ...
        self.arm.move_end_to_pose(new_pos, np.array(new_ori))
        time.sleep(0.1)
        self.base.move_to(np.array([-0.3, 0, 0]), np.array([0,0,0,0]), 'robot', False)
        # print(1)
        ...
        # print(3)
        # self.arm.move_end_to_pose(np.array([0.40, -0.2, 0]), np.array([0.0, 0.0, 0.383, 0.924]))
        # print(4)
    ...
    def close_microwave(self):
        ...
        poses = [(np.array([0.4, 0.1, 0.05]), np.array([0.0, 0.0, 0.0, 1.0])),
                (np.array([0.4, 0, 0.05]), np.array([0.0, 0.0, 0.0, 1.0])),
                (np.array([0.4, -0.1, 0.05]), np.array([0.0, 0.0, 0.0, 1.0])),
                (np.array([0.4, -0.2, 0.05]), np.array([0.0, 0.0, 0.0, 1.0])),
                (np.array([0.4, -0.3, 0.05]), np.array([0.0, 0.0, 0.0, 1.0])),
                (np.array([0.5, -0.4, 0.05]), np.array([0.0, 0.0, 0.0, 1.0])),
                (np.array([0.55, -0.45, 0.05]), np.array([0.0, 0.0, 0.0, 1.0])),
        ]
        for pose in poses:
            print(pose)
            self.arm.move_end_to_pose(*pose)
...
if __name__ == '__main__':
    ...
    print("soultion initial succeed")
    print("to close ")
    s.close_microwave()
