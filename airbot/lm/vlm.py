    ...
    def infer(self, image: np.array ,prompt: Any, **kwargs) -> dict:
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                cls = int(cls.item())
                #TODO old is "dict[cls] == prompt"
                if dict[cls] == prompt:
                    text = prompt #TODO old is = prompt
        ...
        #TODO if detect nothing, init bbox
        if _conf_bbox.nelement() == 0 :
            _conf_bbox = torch.tensor(np.array([0., 0., 0., 0.]))

        # _conf_bbox = max_conf_bbox
        # to y x h w
        bbox = _conf_bbox.clone()
        bbox[1] = (_conf_bbox[0] + _conf_bbox[2]) / 2.  # x center
        bbox[0] = (_conf_bbox[1] + _conf_bbox[3]) / 2.  # y center
        bbox[3] = _conf_bbox[2] - _conf_bbox[0]  # width
        bbox[2] = _conf_bbox[3] - _conf_bbox[1]  # height
        return {
            "bbox": bbox,  # list[np.ndarray(center_h, center_w, height, width)]
            "text": text,  # str
            "score": max_conf  # list[float | np.ndarray]
        }
    def inferss(self, image: np.array ,prompt: Any, lowest_score: float, **kwargs) -> dict:
        '''get the bbox's score heigher than specify score'''
        from yolov7.utils.general import non_max_suppression, scale_coords, xyxy2xywh
        dict = {3: "mug", 2: "microwave_door", 1: "cabinet_handle", 0: "bowl"}
        ...
        if len(det):
            ...
            for *xyxy, score, cls in reversed(det):
                ...
                if dict[cls] == prompt:
                    ...
                    if score.__float__() > lowest_score:
                        bbox_list.append(torch.tensor(xyxy))
                        score_list.append(score)

        # to y x h w
        for bbox in bbox_list:
            bbox_clone = bbox.clone()
            bbox[1] = (bbox_clone[0] + bbox_clone[2]) / 2.  # x center
            bbox[0] = (bbox_clone[1] + bbox_clone[3]) / 2.  # y center
            bbox[3] = bbox_clone[2] - bbox_clone[0]  # width
            bbox[2] = bbox_clone[3] - bbox_clone[1]  # height

        return {
            "bbox": bbox_list,  # list[np.ndarray(center_h, center_w, height, width)]
            "text": text,  # str
            "score": score_list  # list[float | np.ndarray]
        }

class YoloV8_sim(Detector, model='yolo-v8-sim'):
    def __init__(self, model, **kwargs):
        super().__init__(model)
        from ultralytics import YOLO
        self.model = YOLO("/root/Workspace/AXS_baseline/ckpt/yolo-v8/best_sim.pt")
        self.dic = {3: "cabinet_handle", 2: "cup", 1: "gray_bowl", 0: "pink_bowl"}

    def infer(self, image: np.array ,prompt: Any, conf = 0.5, **kwargs) -> dict:
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        boxes = self.model(image, conf = conf)[0].boxes
        conf_bbox = torch.tensor(np.array([0., 0., 0., 0]))
        max_conf = -1

        for b in boxes:
            if self.dic[int(b.cls)] == prompt:
                if b.conf > max_conf:
                    conf_bbox = b.xyxy[0]
                    max_conf = b.conf

        # to y x h w
        bbox = conf_bbox.clone()
        bbox[1] = (conf_bbox[0] + conf_bbox[2]) / 2.  # x center
        bbox[0] = (conf_bbox[1] + conf_bbox[3]) / 2.  # y center
        bbox[3] = conf_bbox[2] - conf_bbox[0]  # width
        bbox[2] = conf_bbox[3] - conf_bbox[1]  # height
    ...
    def inferss(self, image: np.array, **kwargs) -> dict:
        '''get objects in the image'''
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        boxes = self.model(image)[0].boxes
        objects = []
        for i in boxes.cls:
            objects.append(self.dic[int(i)])
        return objects        

class YoloV8(Detector, model='yolo-v8'):
    def __init__(self, model, **kwargs):
        super().__init__(model)
        from ultralytics import YOLO
        self.model = YOLO("/root/Workspace/AXS_baseline/ckpt/yolo-v8/best_real.pt")
        self.dic = {4: 'white_bowl', 3: "cabinet_handle", 2: "cup", 1: "gray_bowl", 0: "pink_bowl"}
