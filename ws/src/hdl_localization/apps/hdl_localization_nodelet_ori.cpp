...
#include <vector>
double total_reg_consumption=0;
int total=0;
using namespace std;
...
// #define DEBUG_FILE_DIR(name)     (string("/home/jzian/WorkSpace/4block_loc/hdl_loc/src/hdl_localization/" + "Log/"+ name))
...
public:
  ...
  void onInit() override {
    ...
    fout_pose.open("/home/user/rosProject/hdl_loc/src/hdl_localization/Log/hdl_pose.txt", ios::out);
...
private:
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    //         Eigen::Vector3d P_L(pcl_cloud->points[i].x, pcl_cloud->points[i].y, pcl_cloud->points[i].z);
    //         Eigen::Vector3d P_gt(gt_R_L * P_L + gt_trans_L);
    //         pcl_cloud->points[i].x = P_gt(0);
    //         pcl_cloud->points[i].y = P_gt(1);
    //         pcl_cloud->points[i].z = P_gt(2);
    //     }
    ...
    if(!use_imu) {
      ...
      for(imu_iter; imu_iter != imu_data.end(); imu_iter++) {
        ...
        // pose_estimator->predict((*imu_iter)->header.stamp, acc_sign * Eigen::Vector3f(-acc.y, acc.x, acc.z), gyro_sign * Eigen::Vector3f(-gyro.y, gyro.x, gyro.z));
        pose_estimator->predict((*imu_iter)->header.stamp, acc_sign * Eigen::Vector3f(acc.x, acc.y, acc.z), gyro_sign * Eigen::Vector3f(gyro.x, gyro.y, gyro.z));
      }
    ...
    publish_odometry(points_msg->header.stamp, pose_estimator->matrix());
           double reg_consumption = (ros::WallTime::now() - reg_t1).toSec() * 1000.0;
           if(reg_consumption < 50) {
            total++;
            std::cout<< "count: "<<total <<"  " << "reg_consumption:"<<reg_consumption<<std::endl;
            total_reg_consumption+=reg_consumption;
            std::cout<<" avg_consumption: "<<total_reg_consumption/total<<std::endl;}
  ...
  void publish_odometry(const ros::Time& stamp, const Eigen::Matrix4f& pose) {
    ...
    rot = Eigen::Quaterniond(odom.pose.pose.orientation.w , odom.pose.pose.orientation.x , odom.pose.pose.orientation.y , odom.pose.pose.orientation.z).toRotationMatrix();
    Eigen::Vector3d pos(odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.position.z);
    Eigen::Matrix3d rot_out = gt_R_L * rot;
    Eigen::Vector3d pos_out = gt_R_L * pos +  gt_trans_L;
    // Eigen::Matrix3d rot_out = rot * gt_R_L;
    // Eigen::Vector3d pos_out = rot * gt_trans_L +  pos;
    Eigen::Quaterniond qua_out(rot_out);
    fout_pose << stamp <<" "\
