...
public:
  ...
  void onInit() override {
    ...
    robot_odom_frame_id = private_nh.param<std::string>("robot_odom_frame_id", "odom"); //robot_odom->velodyne
    odom_child_frame_id = private_nh.param<std::string>("odom_child_frame_id", "livox_frame"); //base->robot
    blind = private_nh.param<std::float_t>("blind", 0.31);
    outer = private_nh.param<std::float_t>("outer", 7.0);
    ...
    points_sub = mt_nh.subscribe("/livox/lidar", 5, &HdlLocalizationNodelet::points_callback, this);
    // points_sub = mt_nh.subscribe("/livox/lidar", 5, &HdlLocalizationNodelet::livox_pcl_cbk, this);

    globalmap_sub = nh.subscribe("/3dmap", 1, &HdlLocalizationNodelet::globalmap_callback, this);  // globalmap -> 3dmap  
...
void livox_pcl_cbk(const livox_ros_driver2::CustomMsg::ConstPtr &msg) 
{
  ...
  pcl::PointCloud<PointT> pl_surf;
  pcl::PointCloud<PointT> pl_buff[128];
  pl_surf.clear();
  // const auto& stamp = msg->header.stamp;
  // const auto& stamp = ros::Time(msg->points[0].offset_time);
  ...
  uint valid_num = 0;
    ...
    for(uint i=1; i<plsize; i++)
    {
      if((msg->points[i].line < 6) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % 2 == 0)
        {
          pl_surf[i].x = msg->points[i].x;
          pl_surf[i].y = msg->points[i].y;
          pl_surf[i].z = msg->points[i].z;
          pl_surf[i].intensity = msg->points[i].reflectivity;
          // pl_surf[i].curvature = msg->points[i].offset_time / float(1000000); // use curvature as time of each laser points, curvature unit: ms

          if((abs(pl_surf[i].x - pl_surf[i-1].x) > 1e-7) 
              || (abs(pl_surf[i].y - pl_surf[i-1].y) > 1e-7)
              || (abs(pl_surf[i].z - pl_surf[i-1].z) > 1e-7)
              && (pl_surf[i].x * pl_surf[i].x + pl_surf[i].y * pl_surf[i].y + pl_surf[i].z * pl_surf[i].z > (blind * blind)))
          {
            (*cloud_out).push_back(pl_surf[i]);
          }
        }
      }
    }
    ...
    if(this->tf_buffer.canTransform(odom_child_frame_id, cloud_out->header.frame_id, stamp, ros::Duration(0.1), &tfError))
    {

        if(!pcl_ros::transformPointCloud(odom_child_frame_id, *cloud_out, *cloud, this->tf_buffer)) {
    ...
    if(aligned_pub.getNumSubscribers()) {
      aligned->header.frame_id = "3dmap";  //map -> 3dmap
      aligned->header.stamp = cloud->header.stamp;
      aligned_pub.publish(aligned);
    }

    if(status_pub.getNumSubscribers()) {
      publish_scan_matching_status(msg->header, aligned);
    }

    publish_odometry(msg->header.stamp, pose_estimator->matrix());
}
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    // std;::cout << extrinT[1] << std::endl;
    // extrinR << 0.007278,  0.999955, -0.006118,-0.709164  ,0.000848 ,-0.705043,-0.705006,  0.009470,  0.709138;
    // extrinT <<
    if(!globalmap) {
...
///////////////////
    ...
    pcl::PointCloud<PointT>::Ptr cloud_out(new pcl::PointCloud<PointT>());
    (*cloud_out).header = (*pcl_cloud).header;
    ...
    for (int i = 0; i < plsize; i++) {
      ...
      p.y = (*pcl_cloud)[i].y;
      p.z = (*pcl_cloud)[i].z;
      p.intensity = (*pcl_cloud)[i].intensity;

      ////////////////////////////////////////////
      // lpf：去掉车身噪声点 长宽高：980*718*330
      // lidar: 接线口方向为x，垂直方向为y
      float EGO_X_MAX = 0.05;
      float EGO_X_MIN = -0.688;
      float EGO_Y_MAX = 0.260;
      float EGO_Y_MIN = -0.260;
      if((p.x < EGO_X_MAX) && (p.x > EGO_X_MIN) && (p.y < EGO_Y_MAX) && (p.y > EGO_Y_MIN)){
          continue;
        }
      ////////////////////////////////////////////

      V3D P_i(p.x, p.y, p.z);
      V3D P_out = Lidar_R_wrt_IMU*P_i + Lidar_T_wrt_IMU;
      p.x = P_out[0];
      p.y = P_out[1];
      p.z = P_out[2];

      double dis = p.x*p.x + p.y*p.y + p.z*p.z;
      // if ((dis > blind*blind) && (dis < outer*outer))
      if ((dis < outer*outer))
      {
    ...
    // std::cout<< "plnum:  "<< cloud_out.size() <<std::endl;
    // auto cloud_out_ptr = cloud_out.makeShared();
    //////////////////////
  ...
  void publish_odometry(const ros::Time& stamp, const Eigen::Matrix4f& pose) {
    ...
    if(tf_buffer.canTransform(robot_odom_frame_id, odom_child_frame_id, ros::Time(0))) {
      ...
      map_wrt_frame.child_frame_id = "3dmap"; //map -> 3dmap
      ...
      odom_trans.header.frame_id = "3dmap"; //map -> 3dmap
    ...
    odom.header.frame_id = "3dmap"; //map -> 3dmap
